...
- 打开网页、点击、填表、截图、观察页面
- 多标签：`tabs` / `new_tab` / `switch_tab` / `close_tab`
- 语义等待：`wait_selector` / `wait_url` / `scroll_into_view`
- 强定位：`click_css` / `click_role` / `click_label`（旧 click_id/text 兼容）
- 上传/下载：`upload` / `download`
- iframe：`frame_enter` / `frame_exit` / `frame_main`
- 可选持久 profile：`BROWSER_PROFILE_DIR`
